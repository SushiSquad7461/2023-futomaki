...
public class PoseEstimator {
  private final SwerveKinematics m_kinematics;
  private final SwerveOdom m_odometry;
  ...
  public Pose2d updateWithTime(double currentTimeSeconds, Rotation2d gyroAngle, SwerveModulePosition[] wheelPositions) {
    m_odometry.updatePoseWithGyro(wheelPositions, gyroAngle);
    m_poseBuffer.addSample(
        currentTimeSeconds,
        new InterpolationRecord(getEstimatedPosition(), gyroAngle, wheelPositions.clone()));
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    public InterpolationRecord interpolate(InterpolationRecord endValue, double t) {
      if (t < 0) {
        ...
        var wheelLerp = new SwerveModulePosition[wheelPositions.length];
        for (int i = 0; i < wheelLerp.length; i++) {
            wheelLerp[i] = new SwerveModulePosition(
                MathUtil.interpolate(wheelPositions[i].distanceMeters, endValue.wheelPositions[i].distanceMeters, t),
                wheelPositions[i].angle.interpolate(endValue.wheelPositions[i].angle, t));
        }
        // wheelPositions.interpolate(endValue.wheelPositions, t);
        ...
        Twist2d twist = m_kinematics.getTwistFromDeltra(wheelLerp);
